...
async function getDictionary(event) {
  ...
  IKAMA_TASUWI = Object.fromEntries(Array.from(itTemp, row => {
    let arr = row.split("\t");
    arr[1].replaceAll(/`(.+)`/g, '<a href="https://nimi.li/$1" target="_blank" class="tp">$1</a>')
    return arr;
  }));
  console.log(IKAMA_TASUWI);

  const TSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vTVGXFd17kcvfu__zjshqiV3kW360IclOEfEdWda_K6ZCg4TY6nW2Gwn4_bs1yQeFLwrZI1_xEvSuP0/pub?gid=0&single=true&output=tsv';
...
<div class="wordHead">
  ...
  <ul class="wordDef">
    ${DICT[i].def.noun ? "<li><strong>Noun:</strong> " + DICT[i].def.noun + "</li>" : ""}
    ${DICT[i].def.verb ? "<li><strong>Verb:</strong> " + DICT[i].def.verb + "</li>" : ""}
    ${DICT[i].def.mod ? "<li><strong>Modifier:</strong> " + DICT[i].def.mod + "</li>" : ""}
  </ul>
  <ul class="wordEtym">
    <li><strong>From:</strong> ${DICT[i].etym.ipa ? DICT[i].etym.word + ' <span class="ipa">' + DICT[i].etym.ipa + '</span>' : DICT[i].etym.word}</li>
    <li><strong>Family:</strong> ${DICT[i].etym.family || "N/A"}</li>
  </ul>
</div>`;
    let img = document.createElement("img");
    img.loading = "lazy";
    img.src = "./img/" + DICT[i].word;
    el.children[0].insertBefore(img, el.children[0].children[0]);
    list.appendChild(el);
  }
  setTimeout(
    () => {
...
document.addEventListener("DOMContentLoaded", () => {
  document.getElementById("getDictionary").addEventListener("click", getDictionary);
})